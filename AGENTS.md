...
- **Send pipeline.** `sendManualTransaction` is simulate at 1.4M CU → limit `ceil(used * 1.1)` capped at 1.4M → `getPriorityFeeEstimate` at `PRIORITY_LEVEL` (default `High`) → `sendRawTransaction(skipPreflight: true)` with 1s poll / 2.5s resend until blockhash expiry, up to `MAX_SEND_ATTEMPTS = 3` with a fresh blockhash per attempt (grace-checks the old signature first). Don't "fix" it to a static limit or preflight-on.
- **Fees.** `getPriorityFeeEstimate` is Helius-only: `PRIORITY_LEVEL=Auto` sends `{ recommended: true }` (Helius picks), any other level sends `{ priorityLevel }` alone — `recommended` can NOT be combined with `priorityLevel` (the API 400s and the bot would silently fall back to 0 fee; a warn now fires on any no-estimate payload). Levels Min..UnsafeMax per https://www.helius.dev/docs/priority-fee-api. Other RPCs warn and fall back to 0 priority fee. Use a Helius `RPC_URL` for the poll loop — `api.mainnet-beta.solana.com` is rate-limited for this.
